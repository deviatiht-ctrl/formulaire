...
- Cache: vercel.json sends Cache-Control: no-cache on everything. All local .css/.js references in HTML carry ?v=YYYYMMDD (current: 20260922); bump the date on every deploy that changes assets, so browsers fetch the new files once and then revalidate cheaply. Never add unversioned local script/link tags.
